...
const sources = ["YouTube", "Twitter", "Reddit", "TikTok", "Twitch", "Discord"];
...
export const generateComments = (activities: ProductionActivity[]): Comment[] => {
    ...
    activities.filter(a => a.status !== "Upcoming").forEach(activity => {
        const activityDate = new Date(activity.date);
        const numComments = activity.type === ProductionActivityType.MarketingCampaign ? 50 : 25;
        
        for (let i = 0; i < numComments; i++) {
            const commentDate = new Date(activityDate.getTime() + Math.random() * 3 * 24 * 60 * 60 * 1000); // within 3 days
            
            let sentiment: Sentiment = "Neutral";
            const rand = Math.random();
            const userType = isPlayerComment(activity) ? 'Player' : 'Viewer';

            if (activity.description.includes("Nerf")) {
                if (rand < 0.7) sentiment = "Negative"; else if (rand < 0.9) sentiment = "Positive";
            } else if (activity.description.includes("Stress Test")) {
                 if (rand < 0.6) sentiment = "Negative"; else if (rand < 0.8) sentiment = "Neutral"; else sentiment = "Positive";
            } else if (activity.type === ProductionActivityType.MarketingCampaign) {
                if (rand < 0.8) sentiment = "Positive"; else if (rand < 0.9) sentiment = "Neutral"; else sentiment = "Negative";
            } else {
                 if (rand < 0.5) sentiment = "Positive"; else if (rand < 0.8) sentiment = "Neutral"; else sentiment = "Negative";
            }

            comments.push({
                id: commentId++,
                activityId: activity.id,
                text: generateCommentText(sentiment, userType, activity),
                author: `${firstNames[Math.floor(Math.random() * firstNames.length)]}${lastNames[Math.floor(Math.random() * lastNames.length)]}${Math.floor(Math.random()*100)}`,
                sentiment,
                userType,
